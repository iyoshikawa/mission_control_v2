...
### Phase 3 — Nav Shell (4 tabs) — 2026-03-13
- Branch/worktree: p3-nav / feat/p3-nav
- Status: DONE
- What changed:
  - Expanded nav from 2 tabs (Dashboard / X Feed) to 4 tabs (Mission Control / X / News / Macro)
  - Renamed tab labels and view IDs: "Dashboard" → "Mission Control" (`view-mission-control`), "X Feed" → "X" (`view-x`)
  - Added News and Macro as placeholder panels with centered text
  - Made `initViewNav()` generic — derives view IDs from tab `data-view` attributes instead of hardcoding
  - Mission Control remains the default active view
  - Updated page title to "Mission Control"
- Files changed:
  - views/index.html — 4 nav tabs, renamed view container IDs, added 2 placeholder `<div>` panels
  - views/app.js — rewrote `initViewNav()` to be generic (~12 lines)
  - views/styles.css — added `.view-placeholder-panel` styles (~15 lines)
- Assumptions:
  - Tab IDs follow the pattern `view-{data-view}` — adding new views requires both an HTML panel and a matching data-view attribute
  - News and Macro are intentionally empty placeholders — other phases will populate them
  - No routing, URL hash, or framework — purely display toggling
- Merge risks:
  - **ID renames**: `view-dashboard` → `view-mission-control`, `view-x-feed` → `view-x`. Any code referencing old IDs (CSS selectors, JS getElementById) needs updating.
  - **initViewNav rewrite**: now generic instead of hardcoded. Any code that depended on the old hardcoded logic needs review.
  - **styles.css**: new `.view-placeholder-panel` class — purely additive, no existing rules modified.
- Recommended review order: index.html (ID renames + new panels), then app.js (generic nav), then styles.css
